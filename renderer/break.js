...
function updateColors(src) {
  ...
  img.onload = () => {
    ...
    if (isDark) {
      ...
      // Dark saturated version of the image's dominant hue
...
// ── State ─────────────────────────────────────────────────────────────────────
window.api.onState(({ eyeState, eyeRemaining }) => {
  if (eyeState !== 'rest') {
    window.api.skip();
    return;
  }
  countdownEl.textContent = eyeRemaining;
});

window.api.onBg((imagePath) => {
  const src = imagePath ? imagePath.replace(/\\/g, '/') : '../assets/bg-1.png';
  containerEl.style.backgroundImage = `url('${src}')`;
  updateColors(imagePath ? `file://${src}` : src);
});

updateColors('../assets/bg-1.png');
